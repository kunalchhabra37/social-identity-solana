...
const main = async () => {
    ...
    const [typeInfoPDAOld, _bumpTypeInfoOld] = PublicKey.findProgramAddressSync(
        [
    ...
    const [typeInfoPDA, _bumpTypeInfo] = PublicKey.findProgramAddressSync(
        [
            ...
            provider.wallet.publicKey.toBuffer(),
            userAccountPDA.toBuffer(),
            anchor.utils.bytes.utf8.encode("twitterz")
        ],
        program_id
    )

    const trx = await program.methods.edit("twitterz", "twitter", "https://twitter.com/kunal_chhabra37").accounts({
        userAccount: userAccountPDA,
        typeInfoAccountOld: typeInfoPDAOld,
